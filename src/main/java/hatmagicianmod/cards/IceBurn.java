...
public class IceBurn extends CustomCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        int final_block = this.magicNumber;
        AbstractPower power = m.getPower(BrandBurnPower.POWER_ID);
        if (power != null && power.amount > 0) {
//            this.addToTop(new ApplyPowerAction(m, p, new BrandBurnPower(m, power.amount)));
            final_block += power.amount;
        }
        if (final_block > 0) {
            this.addToTop(new GainBlockAction(p, final_block));
        }
    }
